<p align="center">
<!-- 로고 이미지-->
</p>
<h1 align="center">47th-web</h1>
<p align="center">
KAPA의 47회 전시회를 위해 제작 배포되는 웹사이트입니다.<br/> 
KAPA 41기 박우석, 42기 최승아가 제작하였습니다.
</p>

---

## 🌐 Link

🔗 [KAPA 47회 정기 전시회 바로가기](https://www.kapaforever.site/)

---

## ✨ Features

- HOME, VISIT, EXHIBITION, GOODS로 구분된 명확한 정보 구조
- Lottie 애니메이션을 활용한 동적인 로고파일 생성
- 모바일 및 데스크탑에 최적화된 반응형 UI
- TailwindCSS 기반의 일관된 디자인 시스템
- Vercel 배포 및 배포 파이프라인 자동화

---

## 📹 Screen Shots

<p align="center">
  <!-- 캡처이미지 -->
</p>

---

## 🛠️ Skills & Tech Stack

### 🖥️ Deploy

![Vercel](https://img.shields.io/badge/Vercel-000000?style=for-the-badge&logo=vercel&logoColor=white)

### 💻 Develop

![React](https://img.shields.io/badge/React-20232A?style=for-the-badge&logo=react&logoColor=61DAFB)
![TypeScript](https://img.shields.io/badge/TypeScript-007ACC?style=for-the-badge&logo=typescript&logoColor=white)
![TailwindCSS](https://img.shields.io/badge/Tailwind_CSS-38B2AC?style=for-the-badge&logo=tailwindcss&logoColor=white)
![React Router](https://img.shields.io/badge/React_Router-CA4245?style=for-the-badge&logo=react-router&logoColor=white)
![Lottie](https://img.shields.io/badge/Lottie-00B3E6?style=for-the-badge&logo=lottie&logoColor=white)

### 🎨 Design

![Figma](https://img.shields.io/badge/Figma-F24E1E?style=for-the-badge&logo=figma&logoColor=white)
![Adobe Illustrator](https://img.shields.io/badge/Adobe_Illustrator-FF9A00?style=for-the-badge&logo=adobe%20illustrator&logoColor=white)


---

# 👫🏻 Contributors

| <img src="https://avatars.githubusercontent.com/u/102500033?v=4" width="200"> | <img src="https://avatars.githubusercontent.com/u/92146550?v=4" width="200"> |
| :---------------------------------------------------------------------------: | :--------------------------------------------------------------------------: |
|                                     우석                                      |                                     승아                                     |
|                  [jamooooong](https://github.com/jamooooong)                  |                     [winA](https://github.com/winnaba61)                     |
|                                   Frontend                                    |                                   FrontEnd                                   |

---

## 📦 주요 패키지

![React](https://img.shields.io/badge/React-20232A?style=for-the-badge&logo=react&logoColor=61DAFB)
![TypeScript](https://img.shields.io/badge/TypeScript-007ACC?style=for-the-badge&logo=typescript&logoColor=white)
![Vite](https://img.shields.io/badge/Vite-646CFF?style=for-the-badge&logo=vite&logoColor=white)
![TailwindCSS](https://img.shields.io/badge/Tailwind_CSS-38B2AC?style=for-the-badge&logo=tailwindcss&logoColor=white)
![React Router](https://img.shields.io/badge/React_Router-CA4245?style=for-the-badge&logo=react-router&logoColor=white)
![Lottie](https://img.shields.io/badge/Lottie-00B3E6?style=for-the-badge&logo=lottie&logoColor=white)
![Vercel](https://img.shields.io/badge/Vercel-000000?style=for-the-badge&logo=vercel&logoColor=white)
![Figma](https://img.shields.io/badge/Figma-F24E1E?style=for-the-badge&logo=figma&logoColor=white)
![Adobe Illustrator](https://img.shields.io/badge/Adobe_Illustrator-FF9A00?style=for-the-badge&logo=adobe%20illustrator&logoColor=white)


---

## 📂 디렉토리 구조
```
src/
 ├── assets/
 │   ├── data/
 │   ├── images/
 │   │   ├── jpg/
 │   │   ├── lottie/
 │   │   └── svg/
 │   │       ├── arrow/
 │   │       │   ├── back/
 │   │       │   └── next/
 │   │       └── close/
 │
 ├── hooks/
 │
 ├── layout/
 │   ├── Footer/
 │   ├── Header/
 │   │   ├── _components/
 │   └── index.tsx
 │
 ├── pages/
 │   ├── GoodsPage/
 │   │   ├── _components/
 │   ├── MainPage/
 │   ├── VisitPage/
 │   │   ├── __components/
 │   └── exhibitionPage/
 │       ├── _components/
 │       ├── fallPage/
 │       ├── springPage/
 │       ├── summerPage/
 │       └── winterPage/
 │
 ├── routes/
 │
 ├── styles/
 │   └── fonts/
 │       └── pretendard/
 │
 └── (기타 루트 파일)
 ```

---

## 🗄️ 백엔드 구조

본 프로젝트는 정적 사이트로 백엔드 서버는 별도로 사용하지 않습니다. <br/>
데이터 및 리소스는 Vercel 배포 환경에 포함되어 관리됩니다.
